...
> 方法论参考：[skills/specfs-port/SKILL.md §阶段-2](../skills/specfs-port/SKILL.md#阶段-2--抽取-sysspec-规范树)。
> 规范文件格式：[skills/specfs-port/references/specfs-format.md](../skills/specfs-port/references/specfs-format.md)。
> 命令本身只描述**编排步骤**——概念性内容请跳转到 SKILL。
...
plus all injected context segments.
...
四段格式与质量门槛见 [SKILL.md §阶段-2](../skills/specfs-port/SKILL.md#阶段-2--抽取-sysspec-规范树)。
...
- Auto-collected sanity check:
  ...
  - **Two-phase lock check** — run a 2-step trigger scan:
    ...
    - If EITHER scan hits → spec MUST contain a `## Refine Prompt` section
      ...
      violation, call `spec_gen_refine(session_id, user_suggestion="Two-phase
      trigger fired (locks present in <Linux source | spec [RELY]>
...
> 两阶段 spec 写法的完整契约见 [SKILL.md §阶段-2](../skills/specfs-port/SKILL.md#阶段-2--抽取-sysspec-规范树)。
...
## Batch mode — 同模块多 stage 并行 spec_gen
